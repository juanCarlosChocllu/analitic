...
export class SucursalService {
  constructor(
    @InjectModel(Sucursal.name, NombreBdConexion.mia)
    private readonly SucursalSchema: Model<Sucursal>,
  ) {}

  findAll(id: string) {
    const sucursal = this.SucursalSchema.find(
      { empresa: new Types.ObjectId(id), flag: Flag.nuevo },
      'nombre ciudad, flag',
    ).exec();
    return sucursal;
  }
  ...
  async buscarSucursalVentas(ventaDto: VentaDto) {
    ...
    for (let sucursal of ventaDto.sucursal) {
      const sucursalVentas: SucursalVentasI[] =
        await this.SucursalSchema.aggregate([
          {
          ...
          },
          {
            $match: {
              'ventas.tipoVenta': {
                $in: ventaDto.tipoVenta.map(
                  (tipoVenta) => new Types.ObjectId(tipoVenta),
                ),
              },
              'ventas.fecha': {
  ...
  async buscarScursal(id: Types.ObjectId) {
    const sucursal = await this.SucursalSchema.findOne({ _id: id }, 'nombre')
      .lean()
      .exec();
